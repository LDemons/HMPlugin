...
public class DifficultyManager {
    ...
    public void applySpiderEffects(Spider spider) {
    ...
    private int getAmplifierForEffect(PotionEffectType type) {
        if (type.equals(PotionEffectType.SPEED)) return 2; // Velocidad III
        if (type.equals(PotionEffectType.STRENGTH)) return 3; // Fuerza IV
        if (type.equals(PotionEffectType.JUMP_BOOST)) return 4; // Salto V
        if (type.equals(PotionEffectType.REGENERATION)) return 3; // Regeneración IV
        if (type.equals(PotionEffectType.RESISTANCE)) return 2; // Resistencia III
